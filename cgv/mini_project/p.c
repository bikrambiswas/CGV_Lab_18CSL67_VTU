...
int top[3]={-1,-1,-1};
int NUM_DISKS=3;
int cnt,counter,speed=4;
int line1=90,line2=85;
float ycoordinate;
int lightflag=0,animationFlag=1,randomColorFlag=0;
...
{
	glColor3f(0,0,0);
	glRasterPos3f(-75,93,-5);
	printString("Move :");
	char str[5];
	sprintf(str, "%d", counter);
	glRasterPos3f(-40,93,-5);
	printString(str);
	glRasterPos3f(-70,80,0);
	printString("Disk");
	char str1[10];
	sprintf(str1, "%d", moves[counter][0]);
	glRasterPos3f(-50,80,0);
	printString(str1);
	glRasterPos3f(-40,80,0);
	printString("from");
	char src[2];
...
void first()
{
	...
	glColor3f(1,0,0);
	strokeString(50,850,0.4,0.4,"Acharya INSTITUTE OF TECHNOLOGY",2);
	strokeString(100,750,0.25,0.25,"DEPARTMENT OF COMPUTER SCIENCE",4);
